...
export default function ScreenShareVideo({ stream, label, muted = false, mirrored = false }) {
  ...
  return (
    <div ref={cardRef} className="screen-share-preview">
      ...
      <video ref={videoRef} autoPlay playsInline muted={muted} style={mirrored ? { transform: "scaleX(-1)", objectFit: "cover" } : undefined} />
      ...
      {isExpanded && (
        <div className="screen-share-fallback-fullscreen" role="dialog" aria-modal="true">
          ...
          <ScreenShareVideo stream={stream} label={label} muted={muted} mirrored={mirrored} />
        </div>
      )}
    </div>
  );
