...
export default function ScreenShareVideo({ stream, label, muted = false, mirrored = false, fill = false }) {
    ...
    <div ref={cardRef} className="screen-share-preview">
      ...
      <video
        ...
        style={{
          ...
          ...(mirrored || fill ? { objectFit: "cover" } : null),
        }}
      />
      ...
      {isExpanded && (
        <div className="screen-share-fallback-fullscreen" role="dialog" aria-modal="true">
          ...
          <ScreenShareVideo stream={stream} label={label} muted={muted} mirrored={mirrored} fill={fill} />
        </div>
      )}
    </div>
  );
